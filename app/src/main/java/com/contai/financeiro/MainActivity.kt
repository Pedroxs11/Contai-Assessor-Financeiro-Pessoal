...
fun ContaiApp(hideValuesByDefault: Boolean = false) {
    ...
    MaterialTheme { Surface(modifier = Modifier.fillMaxSize()) { Column(modifier = Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(24.dp)) {
        Text("Contai", style = MaterialTheme.typography.headlineLarge); Spacer(modifier = Modifier.height(4.dp)); Text("Seu dinheiro, mais claro.", style = MaterialTheme.typography.bodyLarge, color = MaterialTheme.colorScheme.onSurfaceVariant); Spacer(modifier = Modifier.height(24.dp))
        Card(
            modifier = Modifier.fillMaxWidth(),
            shape = androidx.compose.foundation.shape.RoundedCornerShape(20.dp),
            colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant)
        ) { Row(modifier = Modifier.fillMaxWidth().padding(18.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) { Column { Text(when { !notificationAccess -> "Captura sem permissão"; listenerHealthy -> "Captura ativa"; else -> "Captura interrompida" }, style = MaterialTheme.typography.titleMedium); Spacer(modifier = Modifier.height(4.dp)); Text(when { !notificationAccess -> "Ative o acesso às notificações"; listenerHealthy -> "O Contai está acompanhando suas notificações"; else -> "A captura automática precisa de atenção" }, style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant) }; Text(when { !notificationAccess -> "⚠️"; listenerHealthy -> "●"; else -> "⚠️" }, style = MaterialTheme.typography.titleLarge, color = if (listenerHealthy) MaterialTheme.colorScheme.secondary else MaterialTheme.colorScheme.error) } }; Spacer(modifier = Modifier.height(20.dp))
        ...
        Card(
            ...
            border = androidx.compose.foundation.BorderStroke(1.dp, MaterialTheme.colorScheme.primary.copy(alpha = 0.75f))
        ) { Column(modifier = Modifier.padding(horizontal = 22.dp, vertical = 20.dp)) { Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) { Column { Text("Saldo atual", style = MaterialTheme.typography.titleMedium); Spacer(modifier = Modifier.height(3.dp)); Text("Visão geral", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) }; TextButton(onClick = { showValues = !showValues }) { Text(if (showValues) "👁" else "🙈") } }; Spacer(modifier = Modifier.height(12.dp)); Text(if (showValues) formatCurrency(balance) else "R$ ••••••", style = MaterialTheme.typography.headlineLarge); Spacer(modifier = Modifier.height(8.dp)); Text("Saldo entre entradas e despesas confirmadas", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) } }; Spacer(modifier = Modifier.height(12.dp))
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            Card(modifier = Modifier.weight(1f), shape = androidx.compose.foundation.shape.RoundedCornerShape(20.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant)) { Column(modifier = Modifier.padding(18.dp)) { Text("Entradas", color = MaterialTheme.colorScheme.onSurfaceVariant); Spacer(modifier = Modifier.height(8.dp)); Text(if (showValues) formatCurrency(totalIncome) else "R$ ••••••", style = MaterialTheme.typography.titleLarge, color = MaterialTheme.colorScheme.secondary); Spacer(modifier = Modifier.height(4.dp)); Text("Confirmadas", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) } }
